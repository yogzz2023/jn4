...
def perform_clustering_hypothesis_association(tracks, reports, cov_inv):
    ...
    hypotheses = []
    track_ids = []  # New list to store track IDs
    for cluster in clusters:
        ...
        for count in range(base ** num_tracks):
            ...
            for track_idx in cluster:
                ...
                track_ids.append(track_idx)  # Store track ID
            if is_valid_hypothesis(hypothesis):
                hypotheses.append(hypothesis)

    probabilities = calculate_probabilities(hypotheses, tracks, reports, cov_inv)
    return hypotheses, probabilities, track_ids  # Return track_ids along with other information
...
def main():
    ...
    for group_idx, track_group in enumerate(tracks):
        ...
        # Perform clustering, hypothesis generation, and association
        hypotheses, probabilities, track_ids = perform_clustering_hypothesis_association(track_states, reports, cov_inv)
        ...
        for report_idx, track_idx in enumerate(max_associations):
            if track_idx != -1:
                ...
                kalman_filter.initialize_measurement_for_filtering(report[0], report[1], report[2], track_group[report_idx][3])
                kalman_filter.update_step()
                filtered_state = kalman_filter.Sf.flatten()[:3]
                r, az, el = cart2sph(filtered_state[0], filtered_state[1], filtered_state[2])
                updated_states.append((track_group[report_idx][3], r, az, el, track_ids[report_idx]))  # Pass track ID
...
def plot_track_data(updated_states):
    ...
    times, ranges, azimuths, elevations, track_ids = zip(*updated_states)  # Updated to include track_ids
    ...
    # Add track ID text to each point
    for i in range(len(times)):
        plt.text(times[i], ranges[i], f'Track ID: {track_ids[i]}', fontsize=8, color='black')
