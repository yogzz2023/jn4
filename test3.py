...
def mahalanobis_distance(x, y, cov_inv):
    ...
    x = np.array(x[:3])  # Convert tuple to numpy array
    y = np.array(y[:3])  # Convert tuple to numpy array
    delta = y - x
    ...
    for group_idx, track_group in enumerate(tracks):
        ...
        for i, (x, y, z, mt) in enumerate(track_group):
            if i == 1:
                ...
                associated_reports.append((x, y, z, mt))

        # After association, use the associated reports for track update
        hypotheses, probabilities = perform_clustering_hypothesis_association(track_group, associated_reports, cov_inv)
        max_associations, _ = find_max_associations(hypotheses, probabilities, associated_reports)

        for track_idx, report_idx in enumerate(max_associations):
            if report_idx != -1:
                if report_idx < len(associated_reports):  # Check if report index is valid
                    report = associated_reports[report_idx]
                    kalman_filter.initialize_measurement_for_filtering(report[0], report[1], report[2], report[3])
                    kalman_filter.update_step()
